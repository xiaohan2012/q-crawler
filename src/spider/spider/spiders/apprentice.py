...
class ApprenticeSpider(CrawlSpider):
    ...
    rules = (
        Rule(SgmlLinkExtractor(unique=True), callback='parse_item', follow=False),#should it be unique 
    )
    crawled_urls = []
    ...
    def parse_item(self, response):
        ...
        for url_info in url_infos:
            ...
            req = Request (url, priority = priority, #after the request is done, run parse_item to train the apprentice
                           callback = self.parse_item) #this line is extremely important, or items harvest rate drops drastically
